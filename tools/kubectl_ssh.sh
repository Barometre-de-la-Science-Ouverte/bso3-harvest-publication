...
# chmod +x kubectl_ssh.sh
# ./kubectl_ssh.sh node <NODE_ID>
# Create a pod running busybox image on the node
# https://hub.docker.com/_/busybox The Swiss Army Knife of Embedded Linux
# This pod can access some of the node files
# rotated logs files are in /host/var/logs/pods
# To copy the logs run: kubectl cp <BUSYBOX_POD_ID>:/host/var/log/pods ./logs
