Feature: Harvesting
    ...
    Scenario: harvesting for Arxiv
        ...
        When we send a post request with metadata_file="bso-publications-5k.jsonl.gz", total_partition_number="2" to "http://127.0.0.1:5004/harvest_partitions" endpoint
        and we wait "90" seconds
        Then we check that the response status is "200"
        and we check that the response content length is equals to total_partition_number + 1
