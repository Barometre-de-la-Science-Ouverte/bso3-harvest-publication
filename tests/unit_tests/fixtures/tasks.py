...
source_metadata_file_w_no_doi = os.path.join(FIXTURES_PATH, 'bso-publications-5k_w_no_doi.jsonl.gz')
filtered_metadata_filename_w_no_doi = os.path.join(FIXTURES_PATH, 'filtered_' + os.path.basename(source_metadata_file_w_no_doi))
doi_list_w_no_doi = ["10.1158/1538-7445.sabcs21-p1-17-07"]
...
rows_db: list = [
    ProcessedEntry(
        f"doi_{i}", f"uuid_{i}", "1", softcite_version_db, grobid_version_db, harvester_used, domain, url_used,
        harvesting_date, datastet_version_db
    ) for i in range(10)
]
...
for i in range(3):
    ...
    entry: ProcessedEntry = ProcessedEntry(
        f"doi_{i}", fuuid, "1", softcite_version_db, grobid_version_db, harvester_used, domain,
        url_used, harvesting_date, datastet_version_db
    )
